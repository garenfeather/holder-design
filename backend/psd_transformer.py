...
class BinaryPSDTransformer:
    def _clean_layer_name(self, layer_name):
        ...
        # 去除空字节
        clean_name = layer_name.replace('\x00', '')
        
        # 去除其他控制字符
        clean_name = ''.join(char for char in clean_name if ord(char) >= 32 or char in '\t\n\r')
    ...
    def _detect_required_layers(self):
        ...
        for i, layer in enumerate(self.psd):
            # 清理图层名称
            clean_name = self._clean_layer_name(layer.name).lower()
            print(f"  检查图层: '{layer.name}' -> '{clean_name}'")
            if clean_name in self.required_layers:
                found_layers[clean_name] = layer
    ...
    def _calculate_final_positions(self):
        ...
        for layer in self.psd:
            ...
            key_name = sanitize_name(layer.name)
            self.final_positions[key_name] = {
    ...
    def _modify_psd_binary(self):
        ...
        for layer in self.output_psd:
            ...
            if sname in self.final_positions:
                pos_info = self.final_positions[sname]
    ...
    def transform(self):
        ...
        try:
            ...
            print("\n" + "=" * 60)
            print("[SUCCESS] 变换完成!")
            print(f"[FILE] 输入文件: {self.input_path.name}")
            print(f"[FILE] 输出文件: {self.output_path.name}")
            print(f"📐 画布尺寸: {self.original_size[0]}×{self.original_size[1]} → {self.new_size[0]}×{self.new_size[1]}")
            print(f"[TARGET] 中心偏移: X: +{self.center_offset_x}, Y: +{self.center_offset_y}")
            print("=" * 60)
            
            return True
            
        except Exception as e:
            print(f"[ERROR] 变换过程中出现错误: {e}")
            return False
...
def main():
    ...
    try:
        ...
        if not Path(input_psd).exists():
            print(f"[ERROR] 未找到输入文件: {input_psd}")
            return 1
        
        # 创建变换器
        transformer = BinaryPSDTransformer(input_psd)
