...
流程：
1. 读取PSD模板和源图片，按图层形状替换part图层内容
2. 对替换后的PSD进行变换（画布扩展、图层移动、翻转）
3. 输出最终的变换后PSD文件
"""
...
class IntegratedProcessor:
    ...
    def process(self):
        ...
        try:
            ...
            print(f"🔄 处理图层: {len(self.final_layers)} 个")
            print("=" * 60)
...
def main():
    ...
    if len(sys.argv) < 3:
        print("用法: python integrated_processor.py <模板PSD> <源图片> [输出PSD]")
        print("示例: python integrated_processor.py test.psd source.jpg output.psd")
        return 1
    ...
    try:
        # 创建处理器
        processor = IntegratedProcessor(template_file, source_file, output_file)
