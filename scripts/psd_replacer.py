...
class PSDReplacer:
    ...
    def _generate_output_path(self):
        """生成输出文件路径"""
        return self.template_path.with_name(f"{self.template_path.stem}_replaced.psd")
    ...
    def load_and_validate(self):
        ...
        if (self.template.width, self.template.height) != self.source_image.size:
            print(f"❌ 尺寸不匹配: 模板{self.template.width}×{self.template.height} vs 图片{self.source_image.size[0]}×{self.source_image.size[1]}")
            return False
    ...
    def _detect_part_layers(self):
        ...
        for layer in self.template:
            ...
            if layer_name in self.part_layers:
    ...
    def extract_parts(self):
        ...
        for part_name, info in self.layers.items():
            ...
            if not layer_image:
                print(f"  ⚠️  跳过 {part_name}: 无法获取图层图像")
                continue
    ...
    def _write_replaced_psd(self):
        ...
        for layer in template:
            layer_name = layer.name.lower()
            print(f"  处理图层: {layer.name} ({layer_name})")
            
            if layer_name in self.cropped_parts:
                # 使用截取的图片替换part图层
                part_data = self.cropped_parts[layer_name]
                all_layers.append({
                    'name': layer.name,
                ...
                if layer_image:
                    ...
                    if bounds:
                        ...
                        print(f"    ⚠️ 跳过无边界图层: {layer.name}")
                else:
                    print(f"    ⚠️ 跳过无图像图层: {layer.name}")
    ...
    def _save_psd(self, layers):
        ...
        try:
            ...
            try:
                ...
                print(f"  ❌ 降级保存方案也失败: {fallback_error}")
                raise e
    ...
    def replace(self):
        ...
        try:
            ...
            print("\n" + "=" * 60)
            print("✅ 替换完成!")
            print(f"📁 模板: {self.template_path.name}")
            print(f"📁 源图片: {self.source_image_path.name}")
            print(f"📁 输出: {self.output_path.name} ({file_size} bytes)")
            print(f"📐 尺寸: {self.template.width}×{self.template.height}")
            print(f"🔄 已替换: {', '.join(self.cropped_parts.keys())}")
            print("=" * 60)
...
def main():
    ...
    if len(sys.argv) < 3:
        print("用法: python psd_replacer.py <模板PSD> <源图片> [输出PSD]")
        print("示例: python psd_replacer.py template.psd source.jpg output.psd")
        return 1
    ...
    if not Path(source_file).exists():
        print(f"❌ 源图片不存在: {source_file}")
        return 1
    
    try:
        # 创建替换器
        replacer = PSDReplacer(template_file, source_file, output_file)
        
        # 执行替换
        success = replacer.replace()
